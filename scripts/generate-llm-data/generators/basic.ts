...
import { loadBlogPosts, loadSolutions, loadSuccessStories, loadJobs } from '../utils/content-loader';
...
import { buildBlogPostUrl, buildSolutionUrl, buildSuccessStoryUrl, buildJobUrl, buildAbsoluteUrl } from '../utils/url-builder';
import { generateDocumentMetadata, formatMetadataHeaders } from '../utils/metadata-builder';
...
async function generateServicesSection(): Promise<string> {
  ...
  solutions.forEach(solution => {
    const description = solution.frontmatter.description || extractExcerpt(solution.content, 150);
 ...
 */
async function generateRecentBlogPostsSection(count: number = 5): Promise<string> {
  ...
  recentPosts.forEach(post => {
    ...
    // Format date
    const publishedDate = new Date(post.frontmatter.date).toISOString().split('T')[0];
    ...
    const summary = post.frontmatter.excerpt || extractExcerpt(post.content, 200);
...
async function generateSuccessStoriesSummarySection(): Promise<string> {
  ...
  stories.forEach(story => {
    const summary = story.frontmatter.excerpt || extractExcerpt(story.content, 150);
...
async function generateJobsSummarySection(): Promise<string> {
  ...
  jobs.forEach(job => {
    ...
    parts.push(`- ${job.frontmatter.title} (${location}, ${type}): ${buildJobUrl(job.slug)}`);
  });
