 ...
 */
function parseMDXContent<T = any>(
  content: string
 ...
 */
export async function loadBlogPosts(): Promise<
  ParsedMDX<BlogPostFrontmatter>[]
> {
  ...
  try {
    ...
    return posts.filter(
      (post): post is ParsedMDX<BlogPostFrontmatter> => post !== null
    )
  } catch (error) {
...
export async function loadSolutions(): Promise<
  ParsedMDX<SolutionFrontmatter>[]
> {
  const solutionsDirectory = path.join(process.cwd(), 'content', 'solutions')

  try {
    const dirents = await fs.readdir(solutionsDirectory, {
      withFileTypes: true,
    })
 ...
 */
export async function loadSuccessStories(): Promise<
  ParsedMDX<SuccessStoryFrontmatter>[]
> {
