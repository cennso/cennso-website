...
|---|---|---|---|---|
| `check-contrast.py` | Not JSX tags — a separate color/contrast data source, unrelated to how a file's markup is authored. | unchanged | Output byte-identical with the probe present (no file-count line even changed). Nothing in the probe (a `Card`/`Typography`/`Select`/`NextImage` tree with no literal color classes) falls in this checker's scanning surface. | N/A |
...
| `check-keyboard.py` | `.tsx` source, `onClick`-without-`onKeyDown` on non-interactive elements, `tabIndex` misuse. | unchanged | Probe file counted (110→111), no findings. The probe's source contains no raw `onClick`/`tabIndex` — `Select`'s keyboard handling lives inside `@cennso/ui`, outside any static source checker's reach regardless of markup style. This is a pre-existing limitation of static analysis, not new blindness introduced by the design system. | N/A (inherent to static source scanning) |
| `check-enough-time.py` | `.tsx` source, `setTimeout`/`setInterval`/meta-refresh patterns. | unchanged | Probe file counted (110→111), no findings. Nothing in the probe resembles a timer or auto-refresh. | N/A |
| `check-seizures.py` | `.tsx` source, flash/animation-frequency patterns. | unchanged | Probe file counted (111→112), no findings. Nothing in the probe animates. | N/A |
...
| `check-readable.py` | `.tsx` source, but `check_language_of_page` (SC 3.1.1, line ~93) only inspects `_document.tsx`. | unchanged | Probe file counted (60→61), no findings. This checker is gated to a single file (`_document.tsx`) unrelated to component markup; `@cennso/ui` adoption in `components/` or `pages/` doesn't touch its scanning surface at all. | N/A |
| `check-predictable.py` | `.tsx` source, on-focus/on-input auto-behavior and navigation-consistency patterns. | unchanged | File count changed (127→128), no new issues. Nothing in the probe matches its on-focus/on-input/nav-consistency heuristics. | N/A |
...
| `check-autocomplete.py` | `.tsx` source, `<input ... autocomplete=...>` literal. | unchanged | File count changed (112→113), no findings. The probe has no `<input>` element; `Select` isn't in this checker's scope by design (autocomplete values are an `<input>`-specific HTML attribute). | N/A |
...
- **`check-semantic-structure.py`**: heading hierarchy (first-heading-is-h1,
  ...
  at lines 99 and 125 — both match only literal heading tags.)
...
- **`check-input-assistance.py`** newly fails (exit 1) on the probe. Its
  `<select[^>]*>` pattern (case-insensitive) coincidentally matches `Select`,
  `Select.Trigger`, `Select.Content`, and `Select.Item`, reporting the same
  underlying defect (the `Select` genuinely has no accessible name — a real
  SC 3.3.2 issue) four times, three of which are against elements that are not
  form controls at all. It is unreliable in both directions: it will fire on
  non-form sub-elements of any component whose name happens to contain
  "select"/"input"/"textarea" as a substring, and it will miss the identical
  real defect the moment a consuming team renames the import (e.g.
  `import { Select as Dropdown }`).
...
**Proceed to Task 4, with a named exception.**

Fourteen of the fifteen checkers are either genuinely unaffected (nine: contrast,
media, keyboard, enough-time, seizures, input-modalities, readable, predictable,
autocomplete — none of their assertions apply to the markup this migration
changes) or go blind without ever producing a false positive on correct markup
(four: text-alternatives, semantic-structure, navigable, compatible). None of
the fourteen ever asserts something false about correct `@cennso/ui` markup —
they simply stop asserting some things at all. That blindness is exactly the
kind Lighthouse's accessibility audit (required at ≥95% by the constitution,
run against real rendered pages via axe-core) is positioned to catch instead,
because axe-core inspects the actual DOM and computed accessible names/roles
regardless of what JSX produced them — the same reason the four rendered-HTML
validators (`validate-seo.py`, `validate-structured-data.py`,
`check-internal-links.py`, `validate-og-images.py`) are already immune.
Concretely: alt-text presence, heading-hierarchy order, and accessible names on
interactive elements (the three biggest blind spots, from text-alternatives,
semantic-structure/navigable, and compatible respectively) are all standard
axe-core rules (`image-alt`, `heading-order`, `button-name`/`aria-*` rules) that
Lighthouse's `yarn lighthouse` gate already runs on the real page. The claim
"Lighthouse covers it" is being made explicitly here, not assumed: Task 7's
converted page must pass `yarn lighthouse` at ≥95% accessibility as its own
acceptance criterion, which re-verifies these exact three blind spots on real
markup.

One checker does not get this pass: **`check-input-assistance.py`** produces a
genuine false positive by an unreliable mechanism (an accidental substring
match, not real understanding of the `Select` compound component), and it will
be equally unreliable in the other direction on a differently-named component.
Per the constitution, this is a change to *propose*, not one to make in this
task — nothing under `scripts/` is edited here. The proposal: re-point
`check-input-assistance.py`'s form-control detection at rendered HTML
(`.next/server/pages/**/*.html`), following the pattern `validate-seo.py:115`
already uses, rather than teaching it `@cennso/ui`'s specific compound-component
shape (a repair that would need to be redone for every future component and
would still only be reading source, not the actual accessible-name computation
a screen reader performs).

Because the bad case is confined to one checker whose failure mode is "noisy,
not silent" (it already stops Task 7 at `yarn check:all` rather than letting a
real defect through unnoticed), the full re-pointing fallback is **not**
invoked for this phase. Task 7 proceeds — its one converted page is exactly the
fixture the eventual re-pointing work would need anyway — and its author will
need to either add a real accessible name to any `@cennso/ui` `Select` it uses
(the correct fix, independent of the checker) or, if `check-input-assistance.py`
still misfires against correctly-labeled markup once real accessible-name
attributes are present, escalate that specific re-pointing as its own spec and
pull request per the fallback description in the task brief. Phase 4 is not
blocked on that escalation unless it materializes.
